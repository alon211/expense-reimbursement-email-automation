...
def parse_reimbursement_mail(msg, rule_loader: RuleLoader, logger_instance: logging.Logger = None):
    ...
    Returns:
        dict|None: 如果匹配规则，返回包含主题、发件人、日期、正文、匹配规则的字典；否则返回 None
    """
...
def fetch_reimbursement_mails(logger_instance: logging.Logger = None):
    """
    ...
    try:
        ...
        # 发送汇总通知
        if matched_mails:
            push_content = f"共发现{mail_count}封目标邮件（最近{rule_loader.parse_time_range_days}天）：\n"
            ...
            log.info(f"【业务日志】本次检查未发现目标邮件（已检查最近 {rule_loader.parse_time_range_days} 天的邮件）")
